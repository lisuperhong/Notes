...
38.使用线程池能比避免程序创建大量线程带来的性能降低问题并能减少并发线程的数目；下面总结连接线程池时应该做的事：
- 调用Executors类中的静态方法，例如newCachedThreadPool、newScheduledThreadPool等
- 调用submit提交Runnable或Callable对象
- 如果想要取消一个任务，或如果提交一个Callable对象，就要保存好返回的Future对象
- 当不再提交任何任务时，调用shutdown

39.同步器：
- 信号量：为了通过信号量，线程通过调用acquire请求许可，信号量仅维护一个计数，许可的数目是固定的，由此限制通过的线程数量
- 倒计时门栓：让一个线程集等待直到计数变为0
- 障栅：
- 交换器：两个线程在同一个数据缓冲区的两个实例上工作的时候，就可以使用交换器
- 同步队列：是一种将生产者与消费者线程配对的机制
